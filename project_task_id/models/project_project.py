...
class ProjectProject(models.Model):
    ...
    def set_sequences_numbers_for_all_projects(self):
        records = self.env["project.project"].search([("use_scrum","=",True),("project_no","=",False)])
    ...
    def set_project_no_write(self):
        for record in self:
            if not record.project_no and record.use_scrum:
                record.project_no = self.env["ir.sequence"].next_by_code("project.project")
    ...
    def name_get(self):
        ...
        if bool(
            ...
            for project in self:
                if project.project_no and project.use_scrum:
